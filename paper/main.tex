...
\abstract{Relational reasoning—understanding how entities relate through systematic relations such as ontological hierarchy, part-whole relations, and functional roles—is foundational to human cognition. While large language models such as BERT have demonstrated strong empirical \textit{performance} on semantic tasks, it remains unclear whether this success reflects true conceptual \textit{competence} or surface-level statistical association. In this study, I investigate the extent to which BERT encodes abstract relational schemata by examining the model's internal representations of concept pairs across three broad, cognitively-motivated relational categories: \textit{taxonomic}, \textit{mereological}, and \textit{functional} relations. To distinguish between performance and competence, I compare the behavior of BERT in response to a relational classification task with the representational structure of the \texttt{[CLS]} token embeddings. Results from these experiments reveal that pretrained BERT enables high classification accuracy, indicating the presence of latent relational signals. However, concept pairs do not organize by relation type in high-dimensional embedding space until after fine-tuning on a supervised relation classification task. This indicates that relational schemata are not emergent from pretraining alone, but can be induced via task scaffolding. These findings contribute to a growing body of evidence that behavioral performance in language models does not necessarily imply structured conceptual understanding, but that these models can be trained with inductive biases for grounded relational abstraction.
}
